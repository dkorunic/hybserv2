...
/*
MyStrdup()
  duplicates the string 'str' and returns a pointer to the new
string
*/
...
/*
OutOfMem()
  Called when a memory allocation attempt has failed - make a log
...

{
  ...
  DoShutdown((char *) NULL, "Out of Memory");
} /* OutOfMem() */
...
/*
MakeBlock()
  Allocate a new sub block in the heap pointed to by 'heapptr';
If unsuccessful, the program will terminate with an out of memory
error
*/

static void
MakeBlock(Heap *heapptr)

{
  ...
  /*
   * sub->SlotHoles will not actually contain data until
   * an element is deleted, because as long as it is empty
   * we know that we can keep adding elements onto the end
   * of the last element in the sub block, so something like
   * a netjoin will not take many calculations to determine
   * where the next client/channel go in the sub block
   */
  sub->SlotHoles = (void **) MyMalloc(heapptr->ElementsPerBlock * sizeof(void *));
  memset((void *) sub->SlotHoles, 0, heapptr->ElementsPerBlock * sizeof(void *));
  sub->LastSlotHole = 0;

  /*
   * Now actually allocate memory for the elements
   */
  sub->first = (void *) MyMalloc(heapptr->ElementsPerBlock * heapptr->ElementSize);
...
/*
HeapCreate()
  Allocate a block of memory equal to (elementsize * numperblock)
from which smaller blocks can be suballocated.  This improves
efficiency by reducing multiple calls to malloc() during netjoins
etc.
  Returns a pointer to new heap
*/

Heap *
HeapCreate(size_t elementsize, int numperblock)

{
  ...
  /*
   * Now create the first sub block in our heap; hptr->base will
   * point to it
   */
  MakeBlock(hptr);
...
/*
BlockSubAllocate()
  Find an unused chunk of memory to use for a structure, in one
of heapptr's sub blocks, and return a pointer to it; If there
are no free elements in the current sub blocks, allocate a new
block
*/

void *
BlockSubAllocate(Heap *heapptr)

{
  ...
  {
    /*
     * There are no free slots left anywhere, allocate a new
     * sub block
     */
    MakeBlock(heapptr);
  ...
  for (subptr = heapptr->base; subptr; subptr = subptr->next)
  {
    ...
    {
      /*
       * there are no "holes" in the block (ie: there are no
       * empty slots between elements in the block that need
       * to be filled), so just use the slot right after the
       * last element in the block
       */
      offset = (unsigned long) ((heapptr->ElementsPerBlock - subptr->FreeElements) * heapptr->ElementSize);
    #if 0
      debug("ElsPerBlock-FreeEls=[%d], offset=[%d] addy=[%p]\n",
        heapptr->ElementsPerBlock - subptr->FreeElements, offset,
        (void *) (subptr->first + offset));
    #endif

      heapptr->FreeElements--;
      subptr->FreeElements--;
      subptr->LastUsedSlot = (void *) (subptr->first + offset);
    ...
    else
    {
      ...
      /*
       * There is one or more "holes" in the block, where
       * an element was previously deleted, so fill a hole in
       * with this element.  subptr->SlotHoles contains pointers
       * to all of the holes, so use the last index of
       * subptr->SlotHoles as the new slot
       */
    #if 0
      ...
      subptr->SlotHoles[subptr->LastSlotHole - 1] = (void *) NULL;

      if (!ptr)
      {
        for (ii = 0; ii < subptr->LastSlotHole; ii++)
        {
          if (subptr->SlotHoles[ii] != (void *) NULL)
          {
            ptr = subptr->SlotHoles[ii];
            subptr->SlotHoles[ii] = (void *) NULL;
            break;
          }
        }
      }
...
/*
BlockSubFree()
  Opposite of BlockSubAllocate(), instead of finding a free
slot to use for an element, take the given element and remove
it from the sub block
*/

void
BlockSubFree(Heap *heapptr, void *element)

{
  ...
  for (subptr = heapptr->base; subptr; subptr = subptr->next)
  {
    ...
    {
      /*
       * if: firstelement <= element <= lastelement, then
       * element must be in this sub block; add element's
       * address to the SlotHoles array, so we can fill the
       * slot with another element later
       */
      ...
      /*
       * Check if the last used element in the block is the same
       * as the element we are about to delete - if so, don't bother
       * adding 'element' to subptr->SlotHoles[] because 'element'
       * is the last element in the block and so the next element
       * will automatically be added in the same position
       */
      if (subptr->LastUsedSlot == element)
      {
         ...
         * subptr->LastUsedSlot point the slot right before
         * 'element', unless element is the first slot in the
         * block
         */
        ...
        else
        {
          ...
          #endif
            subptr->LastUsedSlot -= heapptr->ElementSize;
            subptr->SlotHoles[index] = (void *) NULL;
          }
        #if 0
      ...
      {
        /*
         * element is in the middle of the block somewhere,
         * add it to the end of subptr->SlotHoles[]
         */

        subptr->SlotHoles[subptr->LastSlotHole] = (void *) element;
        /*
         * subptr->LastSlotHole needs to be incremented since
         * 'element' was stuck at the very end
         */
        subptr->LastSlotHole++;
      #if 0
...
/*
IsHole()
  Determine if 'ptr' is a valid element in sub block 'sub' by
checking if it matches a pointer in sub->SlotHoles[] which
represents deleted elements
Returns index in SlotHoles[] if ptr is found to be deleted,
otherwise -1
*/

static int
IsHole(SubBlock *sub, void *ptr)
