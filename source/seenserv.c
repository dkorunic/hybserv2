...
static void es_joinpart(struct Luser *, int, char **);
...
static struct Command seencmds[] = {
  ...
  { "JOIN", es_joinpart, LVL_ADMIN },
  { "PART", es_joinpart, LVL_ADMIN },
  { 0, 0, 0 }
};
...
void
es_process(char *nick, char *command)

{
  ...
  {
    notice(n_SeenServ, lptr->nick, "Unknown command [%s]",
      arv[0]);
    MyFree(arv);
    return;
  }
...
/*
es_loaddata()
  Load Seen database - return 1 if successful, -1 if not, and -2
...

{
  ...
  while (fgets(line, MAXLINE - 1, fp))
  {
	...
	/* Use of bzero() is depricated. -kre */
        /* bzero(seen, sizeof(aSeen));*/
	memset(seen, 0, sizeof(aSeen));
        ...
        seen->time = atol(av[3]);
        seen->type = type;
        seen->prev = seenp; seen->next = NULL;
        if (seenp) seenp->next = seen;
        seenp = seen;
        if (!seenb) seenb = seen;
        ++seenc;
      }
...
void es_add(char *nick, char *user, char *host, char *msg, time_t time, int type)
{
  ...
  /* Use of bzero() is depricated. -kre */
  /* bzero(userhost, sizeof(userhost));
  bzero(seen, sizeof(aSeen)); */
  memset(userhost, 0, sizeof(userhost));
  memset(seen, 0, sizeof(aSeen));
  strncpy(seen->nick, nick, NICKLEN);
  strncpy(userhost, user, USERLEN); strcat(userhost, "@");
...
static void
es_seen(struct Luser *lptr, int ac, char **av)

{
  ...
  if (strchr(av[1], '*') || strchr(av[1], '?')) {
    ...
    if (sorted[0]->type == 1) {
      ...
      notice(n_SeenServ, lptr->nick, "%s %s (%s) was last seen %s ago, changing nicks",
             sendstr, sorted[0]->nick, sorted[0]->userhost,
...
static void
es_seennick(struct Luser *lptr, int ac, char **av)
...
static void
es_help(struct Luser *lptr, int ac, char **av)
...
static void
es_unseen(struct Luser *lptr, int ac, char **av)
...
static void
es_seenstat(struct Luser *lptr, int ac, char **av)
...
/*
es_joinpart()
  JOINs or PARTs channels
*/ 

static void
es_joinpart(struct Luser *lptr, int ac, char **av)

{
  ...
  {
    notice(n_SeenServ, lptr->nick,
      "Syntax: %s <channel>", av[0]);
    return;
  }

  RecordCommand("%s: %s!%s@%s %s [%s]",
    n_SeenServ,
    lptr->nick,
    lptr->username,
    lptr->hostname, av[0],
  ...
  if (chptr)
    /* Added block to avoid compiler warnings. Yes, I am perfectly aware of
     * `matching last if' rule, but I want to make it perfectly clean
     * -kre */
    {
      if (!strcmp(av[0], "JOIN")) {
